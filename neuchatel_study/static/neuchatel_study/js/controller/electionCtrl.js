...
neuchatelStudyApp.controller("electionCtrl", function ($scope, $routeParams, $location, Backend, $rootScope, $window) {
    ...
    $scope.submitVoteButton = function () {
        Backend.save_timestamp($rootScope.subject, new Date().getTime(), "Neuchatel Study (4): Verify start");
        $location.path('verify/' + $routeParams['id']);
    };
    ...
    $scope.gotoErrorButton = function (errorReason) {
        switch (errorReason) {
            case "verify":
                ...
                Backend.save_timestamp($rootScope.subject, new Date().getTime(), "Neuchatel Study (5.1): Verification Error detected. Finished");
                break;
            case "confirm":
    ...
    $scope.confirmReturnButton = function () {
        Backend.save_timestamp($rootScope.subject, new Date().getTime(), "Neuchatel Study (5.2): Return-Code validated");
